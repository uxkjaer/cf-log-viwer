 ...
 * @param {boolean} opts.live - Whether live mode is enabled
 * @param {object} opts.assets - Embedded asset paths from import with { type: 'file' }
 */
function createServer({ initialLogs = [], appName = '', live = false, assets = {} } = {}) {
  const logs = [...initialLogs];
  const wsClients = new Set();
  let server = null;

  // Pre-load asset content into memory at startup using Bun.file()
  // These paths come from `import ... with { type: 'file' }` in the entry point,
  // which Bun embeds into the compiled binary.
  const ASSETS = {};

  async function loadAssets() {
    if (assets.indexHtml) {
      ASSETS['index.html'] = {
        content: await Bun.file(assets.indexHtml).text(),
        type: 'text/html; charset=utf-8'
      };
    }
    if (assets.styleCss) {
      ASSETS['style.css'] = {
        content: await Bun.file(assets.styleCss).text(),
        type: 'text/css; charset=utf-8'
      };
    }
    if (assets.appJs) {
      ASSETS['app.js'] = {
        content: await Bun.file(assets.appJs).text(),
  ...
  async function start() {
    // Load embedded assets before starting the server
    await loadAssets();
