...
// Read files at startup so they work both in dev (bun run) and compiled binary.
// In compiled mode, __dirname still resolves relative to the embedded FS.
...
function getAssets() {
  if (!ASSETS) {
    ASSETS = {
      'index.html': { content: loadAsset('index.html'), type: 'text/html; charset=utf-8' },
      'style.css': { content: loadAsset('style.css'), type: 'text/css; charset=utf-8' },
      'app.js': { content: loadAsset('app.js'), type: 'application/javascript; charset=utf-8' },
    };
  }
  return ASSETS;
}
